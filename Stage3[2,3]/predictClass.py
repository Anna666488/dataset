...
class PredictProcessor():
	...
	def pck(self, weight, gtJ, prJ, gtJFull, boxL, idlh = 3, idrs = 12):
		...
		Args:
			...
			idrs		: Index of Normalizer (Right Shoulder on PCK, top head on PCKh)
		"""
		for i in range(len(weight)):  #只算可见的关键点，也即标注的，w为1的，
			if weight[i] == 1:
				self.ratio_pck.append(self.pcki(i, gtJ, prJ, idlh=idlh, idrs = idrs))
				self.ratio_pck_full.append(self.pcki(i, gtJFull, np.asarray(prJ / 255 * boxL)))  #这个转换我也是第一次看到？？
				self.pck_id.append(i)
	
	def compute_pck(self, datagen, idlh = 3, idrs = 12, testSet = None):  #为什么感觉这里的参数都差一个数，就是表示对应关键点的id
		...
		Args:
			...
			idrs		: Index of Normalizer (Right Shoulder on PCK, top head on PCKh)
		"""
		datagen.pck_ready(idlh = idlh, idrs = idrs, testSet = testSet)  #testSet???
		self.ratio_pck = []
		self.ratio_pck_full = []
		self.pck_id = []
