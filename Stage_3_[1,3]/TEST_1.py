...
#关键点的颜色设置
RED = (0, 255, 255)
line_color = [[0,229,238],[0,229,238],[0,197,205],[255,255,0],[238,238,0],[255,215,0],[84,255,159],
              [84,255,159],[84,255,159],[84,255,159],[0,229,238],[0,229,238],[0,245,255],[255,255,0],[238,238,0],[255,215,0]]
...
point_pairs = [[0, 1], [1, 2], [2, 6], [3, 6], [4, 3], [5,4],[6,7],[7,8],[8,9],[9,8],[10,11],[11,12],[12,7],[13,7],[14,13],[15,14]]
#评估部分
...
def _crop_img(img, padding, crop_box,ground_points):
    ...
    #[a,b,c,d]
    img_bbox = [crop_box[0] - max_lenght // 2, crop_box[1] - max_lenght // 2, crop_box[0] + max_lenght // 2,crop_box[1] + max_lenght // 2]
...
def vis(img_cv2, points, point_pairs, epoch):
    ...
    for idx in range(len(points)):
        points_1 = []
        points_1.append(int(points[idx][1]))
        points_1.append(int(points[idx][0]))
        cv2.putText(img_cv2_copy_1, str(dataset_joint[idx]),tuple(points_1),cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,255), 1, cv2.LINE_AA)
        ...
        #cv2.imwrite('E:/2020.7.18/aug_7/small_paper/original/' + str(item) + '.jpg', img_cv2_copy_1)
            # Draw Skeleton
    i = 0
    for pair in point_pairs:
        partA = pair[0]
        partB = pair[1]
        if list(points[partA])[0] != -1 and list(points[partA])[1] != -1 and list(points[partB])[0] != -1 and list(points[partB])[1] != -1:
            points_2 = []
            points_2.append(int(points[partA][1]))
            points_2.append(int(points[partA][0]))
            points_3 = []
            points_3.append(int(points[partB][1]))
            points_3.append(int(points[partB][0]))

            cv2.line(img_cv3_1,
                    tuple(points_2),
                    tuple(points_3),
                    color=tuple(line_color[i]), thickness=15)
            #cv2.putText(img_cv3_1, str(dataset_joint[i]), tuple(points_2), cv2.FONT_HERSHEY_DUPLEX, 1, (255, 0, 255), 1,
            #            cv2.LINE_AA)
    ...
    cv2.imwrite('E:/2020.7.18/aug_7/small_paper/Heatmap/' + str(item) + '.jpg', img_cv3_1)
    #以前是把它放在一起显示的，感觉有点小，我准备把它分开
        #plt.imshow(img_cv3_1)
        #plt.axis("off")
    #plt.savefig('E:/2020.7.18/aug_7/small_paper/heatmap/' + str(epoch))
    ...
    plt.axis("off")
    plt.savefig('E:/2020.7.18/aug_7/results/heatmap_2/' + str(epoch) )
...
if __name__ == '__main__':
    ...
    valid_data_file = 'E:/xiao/Data_new_7.16/Final_datasets/Test_final.txt'
    #'E:/xiao/Data_new_7.16/Annotation_txt/Test.txt' 训练在数据增强后的test
    # 'E:/animal_datase/animalpose_image_part2/test/test.txt' #'F:/wenxin_/T_trrain_D.txt'#'  #'E:/Expermin/1.txt（之前跑的测试内容） '#'E:/jiayou/1.txt'
    ...
    while i in range(len(test_name)):
        img_load = 'E:/xiao/Data_new_7.16/AU_all/Data_all'
    ...
    for item in range(len(test_name)):
        ...
        for m in range(16):
            ...
            predictions_1[m][0] = ((predictions_1[m][0]/ 256) * (box_final[item][3] - box_final[item][1])) + box_final[item][1]- padding[item][0]
            predictions_1[m][1] = ((predictions_1[m][1]/256) * (box_final[item][2] - box_final[item][0])) + box_final[item][0]

        i = 0
        x = cv2.imread(os.path.join('E:/xiao/Data_new_7.16/AU_all/Data_all',test_name[item]))
        ...
        cv2.imwrite('E:/2020.7.18/aug_7/results/original/train_2/' + str(item) + '.jpg', x )
        '''
