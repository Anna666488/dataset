...
class Inference():
	...
	"""
	#hg_refined_tiny_200
	def __init__(self, config_file='config.cfg', model = 'E:/xiao/Data_new_7.16/STAGE3_NEW/Stage_3_[1,3]/Save3_conect1and3_2/hg_network_3', yoloModel = 'E:/xiao/Data_new_7.16/STAGE3_NEW/Stage_3_[1,3]/YOLO_small.ckpt'):
		#'model : E:/results/model_3/hg_network_152' yoloModel = 'D:/home_program/xiangmu_1_1/YOLO_small.ckpt'  #32
	...
	def predictHM(self, img):
		...
		"""
		#return self.predict.pred(self, img / 255)
		return self.predict.pred(img / 255,debug = False,sess=None)  # TypeError: pred() got multiple values for argument 'debug'
	# ------------------------- Joint Prediction -------------------------------
	...
	def predictJoints(self, img, mode = 'cpu', thresh = 0.2):
		...
		if len(np.array(img).shape) == 3:
			...
			SIZE = True
		elif len(np.array(img).shape) == 4:
			print('len(np.array(img).shape):',len(np.array(img).shape))
			batch = np.copy(img)
			SIZE = True
		if SIZE:
			if mode == 'cpu':
				return self.predict.joints_pred_numpy(batch / 255, coord = 'img', thresh = thresh, sess = None) #这里我也对应修改了
			elif mode == 'gpu':
				return self.predict.joints_pred(batch / 255, coord = 'img', debug = False, sess = None)  #这里我进行修改了，将coord='img'改为coord='heatmap'
			else :
